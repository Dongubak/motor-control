"""
homing_test.py — 현재 테스트 환경용 Z축 Homing 테스트
...
동작 방식:
  - 슬레이브 0만 드라이브 내부 Homing Mode(6)로 하드웨어 원점 탐색 수행
  - 슬레이브 1은 Homing 중 현재 위치를 유지한다 (이동하지 않음)
  - 슬레이브 0 Homing 완료 후, 슬레이브 1의 현재 위치를 소프트웨어 원점으로 설정

실행 전 필수 확인:
  - 슬레이브 1은 실행 전에 수동으로 슬레이브 0과 동일한 물리적 높이에 위치시킬 것
  - 슬레이브 0의 하단 리미트 스위치(POT/DI1) 배선 확인
...
# Homing 실행 (슬레이브 0 단독)
# ─────────────────────────────────────────────────────────────
def _run_homing_single(master, all_slaves, hw_idx: int,
                       cycle_time_s: float, timeout_s: float) -> bool:
    """
    슬레이브 hw_idx 한 대만 하드웨어 Homing 수행.
    나머지 슬레이브는 Operation Enabled 상태로 현재 위치 유지.

    완료 조건 : Statusword bit12 = 1 (Homing Attained)
    실패 조건 : Statusword bit13 = 1 (Homing Error) 또는 타임아웃
    """
    hw_slave = all_slaves[hw_idx]

    print(f"\n[Homing] 슬레이브 {hw_idx} 단독 Homing 시작 (bit4=1)...")
    print(f"         슬레이브 {[i for i in range(len(all_slaves)) if i != hw_idx]}"
          f" 는 현재 위치 유지")

    # 첫 프레임: hw_slave → Homing Start, 나머지 → Enable Operation
    for i, slave in enumerate(all_slaves):
        _write_cw(slave, CW_HOMING_START if i == hw_idx else CW_ENABLE_OPERATION)
    master.send_processdata()
    master.receive_processdata()
    ...
    while True:
        ...
        sw  = _read_sw(hw_slave)
        pos = _read_pos(hw_slave)
        bit12 = bool(sw & (1 << 12))   # Homing Attained
        bit13 = bool(sw & (1 << 13))   # Homing Error

        # hw_slave Controlword 갱신
        if bit13:
            print(f"\n[오류] 슬레이브 {hw_idx}: Homing Error!"
                  f" Statusword=0x{sw:04X}")
            _write_cw(hw_slave, CW_ENABLE_OPERATION)
        elif bit12:
            _write_cw(hw_slave, CW_ENABLE_OPERATION)
        else:
            _write_cw(hw_slave, CW_HOMING_START)
        ...
        # 상태 표시
        pos_other = _read_pos(all_slaves[SW_ORIGIN_IDX])
        status = "완료" if bit12 else ("에러" if bit13 else "탐색중")
        print(f"\r  [{elapsed:5.1f}s]  "
              f"M{hw_idx}(HW):{status}(pos={pos:+d})  "
              f"M{SW_ORIGIN_IDX}(SW):정지(pos={pos_other:+d})",
              end='', flush=True)
    ...
    try:
        ...
        for i, slave in enumerate(all_slaves):
            ...
            if i == HW_HOMING_IDX:
                ...
                # 리미트 없음 → CSV 모드로 현재 위치 유지
                _set_mode(slave, 9)   # CSV Mode
                mode_act = struct.unpack('<b', slave.sdo_read(0x6061, 0))[0]
                print(f"    [모드] CSV(9) 설정 → 실제={mode_act}  (리미트 없음, 정지 대기)")
        ...
        print(f"\n  현재 위치:")
        print(f"    M{HW_HOMING_IDX}(HW): {pos0:+d} pulse")
        print(f"    M{SW_ORIGIN_IDX}(SW): {pos1:+d} pulse")
        ...
        print(f"      수동으로 위치시킨 후 Enter 를 누를 것")
        ...
        # ── 6단계: 하드웨어 Homing 실행 (슬레이브 0) ──
        print(f"\n[6단계] 슬레이브 {HW_HOMING_IDX} 하드웨어 Homing 실행")
        homing_ok = _run_homing_single(
            master, all_slaves, HW_HOMING_IDX, CYCLE_TIME_S, HOMING_TIMEOUT_S
        )
        ...
        if homing_ok:
            ...
            # CSV 모드 복귀 (슬레이브 0)
            print(f"\n  CSV Mode(9)로 복귀 중...")
            _set_mode(all_slaves[HW_HOMING_IDX], 9)
            mode_act = struct.unpack('<b', all_slaves[HW_HOMING_IDX].sdo_read(0x6061, 0))[0]
            print(f"    M{HW_HOMING_IDX}: 실제 모드={mode_act}")
            time.sleep(0.1)
